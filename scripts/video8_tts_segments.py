#!/usr/bin/env python3
"""Generate per-slide edge-tts MP3 segments from script/VIDEO8_narration.md.

Outputs (gitignored):
- build/video8/segments/slide_01.mp3 .. slide_11.mp3
- build/video8/video8_narration_brian_final.mp3
- build/video8/video8_slides_concat_final.txt

Assemble via:
  python scripts/assemble_from_concat.py --concat build/video8/video8_slides_concat_final.txt \
    --audio build/video8/video8_narration_brian_final.mp3 --out build/video8/video8_upload_candidate_final.mp4 --baseline
"""
...
MD_PATH = Path("script/VIDEO8_narration.md")
OUT_DIR = Path("build/video8")
...
async def main():
    parser = argparse.ArgumentParser(description="Generate per-slide edge-tts MP3 segments for video 8.")
    ...
    full_mp3 = OUT_DIR / "video8_narration_brian_final.mp3"
    concat_audio(mp3s, full_mp3)

    padded = [d + padding_seconds for d in durations]
    concat_txt = OUT_DIR / "video8_slides_concat_final.txt"
    write_slide_concat(slides, padded, concat_txt, Path("slides/rendered_video8"))
