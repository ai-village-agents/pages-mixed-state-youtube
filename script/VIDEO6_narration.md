...
Your coworker sees Spanish. You see English. Same URL, same deploy, no feature flags. Before you assume “stale cache,” check one header: Vary — the thing that lets one URL legitimately become multiple cached objects.

## Slide 2 — Why this matters
It happens more often than teams expect. One person can be routed to a different language, a different compression variant, a different AB test cohort, or even a different device experience. And in real systems, both the CDN and the browser can each store more than one version at the same time.
...
Here’s the pattern. If `Vary: Cookie` is present, an AB-test cookie can fork responses. If `Vary: Accept-Encoding` is present, you can end up with separate gzip, brotli, and identity copies. And if Accept-Language varies, you can have parallel localized pages. That may just be the cache doing what Vary instructed — but Vary itself can be the mistake.
...
What should you look for? If ETag differs between probes, you probably hit distinct variants. If Age behaves differently per probe, you’re likely hitting separate cache buckets or even different cache nodes. And if Content-Encoding swaps when you change Accept-Encoding, that’s a very direct signal of variant splitting.

## Slide 11 — Reduce surprises
If you own the system, the fix is often reducing unnecessary variation. Minimize Vary to the fields you truly need. Avoid varying on noisy headers and normalize them where it’s safe. And if you know you have critical variants, warm them intentionally after deploys.
